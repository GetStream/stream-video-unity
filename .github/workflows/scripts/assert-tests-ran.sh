...
test_case_count="$(grep -oE 'testcasecount="[0-9]+"' "${results_file}" | head -n 1 | grep -oE '[0-9]+' || true)"
...
if [ "${test_case_count}" -eq 0 ]; then
  echo "::error::The test runner executed 0 tests in ${results_file}. Treating this as a failure - see the log for license or compilation errors."
  exit 1
fi

echo "Test runner executed ${test_case_count} tests in ${results_file}."
