...
Once you open the project, open the `AudioRoomsTutorial` scene in the `Scenes` folder.
...
4. Open the `AudioRoomsManager.cs` script in your code editor and replace the contents with the following script:
```csharp
using System;
...
Let's go through the content of this method line by line.
...
![Game Objects to reference in the scene hierarchy](../assets/tutorials/audio-rooms/07.png)
